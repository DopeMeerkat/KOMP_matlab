function find_registration_errors_4_UCHC_DIC(varargin)
...
for i=1:999
    ...
    ResultFileName=['DIC_BeadsCheckResult',c,'.txt'];
...
fprintf(fileID,'%s\n\n',[[Exp_name,home_dir(end-3:end-1)], ' DIC Beads Check Results']);
aa=dir([home_dir,'01_Submitted\Layers\Images\*_regD.mat']);
for i=1:length(aa)
    n=aa(i).name;
    ind=strfind(n,'_regD.mat');
    temp=MakeBeadCheckReport([home_dir,'01_Submitted\Layers\Images\',n]);
...
switch reg_error_code
    case 0
        er=[G, ': ',S, ': Beads look OK']
    case 1
        er=[G, ': ',S, ': Green beads not found']
    case 2
        er=[G, ': ',S, ': Red beads not found']        
    case 3
        er=[G, ': ',S, ': Green/Red beads not found']
    case 4
        er=[G, ': ',S, ': DIC Image not found']
end

function [reg_info]=registration_KOMP_8_v2channel(varargin)
...
%                        c13=[c,num2str(s),'_B_s',nn,'c2.jpg'];                % green beads/TB
                        c12=[c,num2str(s),'_B_s',nn,'c1.jpg'];                % TB
                        c13=[c,num2str(s),'_B_s',nn,'c2.jpg'];                % green beads/TB
                        c14=[c,num2str(s),'_A_s',nn,'c2.jpg'];                % green beads/AP
...
%                     end
                    ...
                    common_background=uint8((Host1==128)&(red1==128))*128;
...
%                         ref=[direc,c5];
%                         im=[direc,c7];
%                         [reg_info, no_bead, reg_error_code, image_after_rough_reg]=find_main_shift_6D(ref, im, gutta, rot_angle, 0);
                        [no_beadR, Rimage_after_rough_reg]=find_beads(red1, rot_angle, 0);
                        [no_beadG, Gimage_after_rough_reg]=find_beads(Host1, rot_angle, 0);
%                         reg_error_code=reg_error_code+10;
                        if no_beadG==0 & no_beadR==0
                            reg_info=0;
                        elseif no_beadG==1 & no_beadR==0
                            reg_info=1;
                            eval(['imwrite(Gimage_after_rough_reg,''',out_direct1,co,'_DIC_GreenBeads.jpg'',''jpg'',''quality'',100);'])
                        elseif no_beadG==0 & no_beadR==1
                            reg_info=2;
                            eval(['imwrite(Gimage_after_rough_reg,''',out_direct1,co,'_DIC_RedBeads.jpg'',''jpg'',''quality'',100);'])
                        elseif no_beadG==1 & no_beadR==1
                            reg_info=3;
                            eval(['imwrite(Gimage_after_rough_reg,''',out_direct1,co,'_DIC_GreenBeads.jpg'',''jpg'',''quality'',100);'])
                            eval(['imwrite(Gimage_after_rough_reg,''',out_direct1,co,'_DIC_RedBeads.jpg'',''jpg'',''quality'',100);'])
                        end
...
% end
% if ~no_reg_info
    eval(['save ''', out_direct1,co,'_regD.mat'' reg_info;']);
% end
return

function varargout=shift_TRAP_AP_to_DIC(varargin) 
% function shift_both_images(dis_v, dis_h, DIC1, red1, Host1,
dis_v=varargin{1};
dis_h=varargin{2};
if nargin==6        %  -->(TRAP)
    no_ref=3;
    no_test=1;
elseif nargin==8    % -->(AP & DAPI)
    no_ref=4;
    no_test=2;
...
function [no_bead, aa]=find_beads(ref, rot_angle, ap)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%   Find the discrepancies of the ref and im using goota
%
...
temp=sum(a1,2);
m=[];
for i=2000:100:min(4000,floor(size(a1,1)/100)*100-1000)
    ind=round(i/100)-19;
    m=[m, sum(temp(i:i+1000))];
end
ind=find(m==min(m));
if ~isempty(ind)
    if isempty(find(m==0))
        ...
        a1((ind+19)*100:(ind+19)*100+1000,:)=false(1001,size(a1,2));
    end
end
temp=sum(a1,1);
m=[];
...
% disk_size=[250, 300];
disk_size=[100, 150, 200, 250, 300, 350, 400];
for i=1:length(disk_size)
    g2=imdilate(temp,strel('disk',disk_size(i)));
    [L, n]=bwlabel(g2); clear g2
    stat=regionprops(L, 'BoundingBox','Image','Area'); clear L
...
function [result]=MakeBeadCheckReport(r_info)
load(r_info);
if reg_info==0
    result=['Beads look OK']; 
elseif reg_info==1  % Green Beads not found
    result=['Green Beads not found']; 
elseif reg_info==2  % Red Beads not found
    result=['Red Beads not found']; 
elseif reg_info==3  % Green Beads and Red beads not found
    result=['Green Beads and Red beads not found']; 
elseif reg_info==4  % No not found
    result=['DIC Image not found'];
end
