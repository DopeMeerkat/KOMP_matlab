function find_thresholding_values_KOMP_test_for_BioHead_2(direct, exp_name, exp_type, labels, GFP, bt, bn, section, delimeter)
    ...
    %                 parfor j=1:length(bone_number)
                for s=section
                    ...
                    for loop=[1 2 3 4 6 7]%channel            % loop : 1->green, 2->red, 3->DIC, 4->TRAP, 6->AP, 7->DAPI, 9-->GFP(CFP)
    ...
    %                    elseif strcmp(exp_type,'F')
                            eval(['file_name=''',direc,phr1, 'L',num2str(s),'_s',bone_number{k},'_',num2str(loop-1),'_shift3.jpg'';']);
    %                    end
    %                     eval(['file_name=''',direc,phr1, '0' ,num2str(bone_number(j)),'_',num2str(s),'_',num2str(loop-1),'_shift2.jpg'';']);
                        ...
                        if isempty(a)==0
                            ...
                            eval(['file_name1=''',direc,phr1, 'L',num2str(s),'_s',bone_number{k},'_',num2str(loop-1),'_shift2.jpg'';']);
                            a=dir(file_name1);
                            if isempty(a)==0
                                eval(['im=imread(''',direc,a.name,''');'])
                                copyfile(file_name1,file_name)
                                if loop==6
                                    eval(['file_name_NoDAPI=''',direc,phr1, 'L',num2str(s),'_s',bone_number{k},'_shift2_NoDAPI.jpg'';']);
                                    eval(['file_name_NoDAPI1=''',direc,phr1, 'L',num2str(s),'_s',bone_number{k},'_shift3_NoDAPI.jpg'';']);
                                    copyfile(file_name_NoDAPI,file_name_NoDAPI1)
                                end
                            else
                                continue
                            end                        
    ...
    %                         [d, thre] = double_step_threshold_2(im,dic);
    %                         if loop==7
                        if loop==8
