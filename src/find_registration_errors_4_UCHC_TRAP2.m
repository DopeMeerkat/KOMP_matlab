function find_registration_errors_4_UCHC_TRAP2(varargin)
...
for i=1:999
    ...
    ResultFileName=['TRAP_RegistrationResult',c,'.txt'];
...
fprintf(fileID,'%s\n\n',[[Exp_name,home_dir(end-3:end-1)], ' TRAP Registration Results']);
% aa=dir([home_dir,'01_Submitted',delimeter,'Layers',delimeter,'Images',delimeter,'*_reg.mat']);
% str=[Exp_name,'_h',tissue_type,'_'];
% for i=1:length(aa)
%     n=aa(i).name;
%     ind=strfind(n,'_reg.mat');
%     temp=MakeRegistrationReport([home_dir,'01_Submitted',delimeter,'Layers',delimeter,'Images',delimeter',n]);
...
%     str=[str,bt,'L'];
    for j=1:8
        bn=num2str(j);
%         str=[str,bt,'L',section,'_s']
        for k=1:3
            section=num2str(k);
            str=[Exp_name,'_h',tissue_type,'_',bt,'L',section,'_s',bn,'_reg.mat'];
            
            ind=strfind(str,'_reg.mat');
            temp=MakeRegistrationReport([home_dir,'01_Submitted',delimeter,'Layers',delimeter,'Images',delimeter',str]);
        %     result(i,1)=mat2cell(n(1:ind-1),1,length(n(1:ind-1)));
        %     result(i,2)=mat2cell(temp,1,length(temp));

            fprintf(fileID,'%s\t',str(ind-6:ind-1));
            fprintf(fileID,'%s\n',temp);
            clear str
        end
    end
...
if ~isempty(strfind(n,'\'))
    delimeter='\';
elseif ~isempty(strfind(n,'/'))
    delimeter='/';
end
del=strfind(n,delimeter);
...
if  reg_error_code>=60
%     S='SO';
%     reg_error_code=reg_error_code-60;
    if reg_error_code==60
        er=[G, ': DIC image does not exist'] 
    elseif reg_error==61
        er=[G, ': TRAP/AP image does not exist']         
%     elseif reg_error==62
%         er=[G, ': AP image does not exist']  
    end
elseif reg_error_code>=50 & reg_error_code<60
    S='SO';
...
temp2=sum(temp,2);
m=[];
for i=2100:300:min(18000,floor(size(d1,1)/100)*100-1000)
    ind=round(i/300)-6;
    m=[m, sum(temp2(i:i+300))];
end
% m(m<sum(m)*0.01)=1000000;
ind=find(m==min(m));
if ~isempty(ind)
    if isempty(find(m==0))
        if length(ind)~=1
        ...
        temp((ind+6)*300:(ind+6)*300+2000,:)=false(2001,size(d1,2));
    end
end
if length(ind)~=1
    ind=ind(floor(length(ind)/2));
...
% temp1=sum(d1,1);
% m=[];
% for i=2100:300:min(10000,floor(size(d1,2)/100)*100-1000)
%     ind=round(i/300)-6;
%     m=[m, sum(temp1(i:i+300))];
% end
% % m(m<sum(m)*0.01)=1000000;
% ind=find(m==min(m));
% if ~isempty(ind)
...
%             ind=ind(floor(length(ind)/2));
%         end
% %         temp(:,(ind+6)*300:(ind+6)*300+2000)=false(size(d1,1),2001);
%     end
% end
% if length(ind)~=1
%     ind=ind(floor(length(ind)/2));
...
aaaa=reshape([stat.BoundingBox],[4,length([stat.BoundingBox])/4]);
candidate=aaaa(2,:);
tt=(ar/sum(ar)>0.04);
indextemp=find(tt);
% candidate=candidate(ar/sum(ar)>0.04);

top=candidate<median(unique(candidate(tt)));
indtop=find(top);
artop=ar(indtop);
indtop1=find(artop==max(artop));
ind=indtop(indtop1);
st_x=round(stat(ind).BoundingBox(2))+round(disk_size(i)/2);
st_y=round(stat(ind).BoundingBox(1))+round(disk_size(i)/2);
ed_x=min(max(3000,stat(ind).BoundingBox(4))+st_x-1-round(disk_size(i)/2), size(a1,1)-round(disk_size(i)/2));
ed_y=min(max(3000,stat(ind).BoundingBox(3))+st_y-1-round(disk_size(i)/2), size(a1,2)-round(disk_size(i)/2));
% a2(st_x:ed_x,st_y:ed_y)=stat(ind).Image;
first_Bounding_Box=[st_x,ed_x, st_y, ed_y];

bottom=candidate>median(unique(candidate(tt)));
indbottom=find(bottom);
arbottom=ar(bottom);
indbottom1=find(arbottom==max(arbottom));
ind=indbottom(indbottom1);
st_x=round(stat(ind).BoundingBox(2))+round(disk_size(i)/2);
st_y=round(stat(ind).BoundingBox(1))+round(disk_size(i)/2);
ed_x=min(max(3000,stat(ind).BoundingBox(4))+st_x-1-round(disk_size(i)/2), size(a1,1)-round(disk_size(i)/2));
ed_y=min(max(3000,stat(ind).BoundingBox(3))+st_y-1-round(disk_size(i)/2), size(a1,2)-round(disk_size(i)/2));
...
if ~isempty(find(aa==10000))
    result=['No DIC Image'];
elseif ~isempty(find(aa==1000))
    result=['No TRAP Image'];
end
